...
This file creates regression models for the USFS and goodness of fit routines from CV tests
...
combined <- combined[combined$agency =="USFS",]

#cut to years of observations
combined <- combined[combined$FY <= 2022,]
...
## create random effects models for USFS using lme4 package (log-log) both wtih and without FY factor

```{r echo = T, results="hide",warning=FALSE }

reg_covariates_re_loglog=lmer(log(visits)~temp+log_pop30+log_popin+factor(region)+factor(month)+factor(FY)+(1|siteid), data=combined)

reg_airsage_re_loglog=lmer(log(visits)~log_airsageud+temp+log_pop30+log_popin+factor(region)+factor(month)+factor(FY)+(1|siteid), data=combined)

reg_flikr_re_loglog=lmer(log(visits)~log_pud+temp+log_pop30+log_popin+factor(region)+factor(month)+factor(FY)+(1|siteid), data=combined)

reg_twitter_re_loglog=lmer(log(visits)~log_tud+temp+log_pop30+log_popin+factor(region)+factor(month)+factor(FY)+(1|siteid), data=combined)

reg_aud_re_loglog=lmer(log(visits)~log_aud+temp+log_pop30+log_popin+factor(region)+factor(month)+factor(FY)+(1|siteid), data=combined)

reg_eud_re_loglog=lmer(log(visits)~log_eud+temp+log_pop30+log_popin+factor(region)+factor(month)+factor(FY)+(1|siteid), data=combined)

reg_allsources_re_loglog=lmer(log(visits)~log_airsageud+log_pud+log_tud+log_aud+log_eud+temp+log_pop30+log_popin+factor(region)+factor(month)+factor(FY)+(1|siteid), data=combined)

reg_socialmedia_re_loglog=lmer(log(visits)~log_pud+log_tud+log_aud+log_eud+temp+log_pop30+log_popin+factor(region)+factor(month)+factor(FY)+(1|siteid), data=combined)


# create random effects models for USFS using lme4 package (log-linear) needed to drop year factor for the second CV
...
### use case: use model to predict visitation for other forests not sampled in NVUM year rotations (assuming we know nothing about the left out forests, ie no years of counts)
...
### use model to replace NVUM after calibration period- this will also be sort of a temporal stability test

```{r echo = T, results="hide",warning=FALSE }
#list of models specifications to test
model_list2 = list(formula(reg_covariates_re_loglog2),formula(reg_airsage_re_loglog2), formula(reg_flikr_re_loglog2), formula(reg_twitter_re_loglog2), formula(reg_aud_re_loglog2), formula(reg_eud_re_loglog2), formula(reg_allsources_re_loglog2), formula(reg_socialmedia_re_loglog2))

#name them
names(model_list2) = c("covariates","airsage","flikr","twitter","alltrails","ebird","allsources","socialmedia")
```

```{r echo = T,warning=FALSE }
#create CV test for USFS models with fit early years predict later years, train length varying 1,2,3,4 years. 
years=sort(unique(combined$FY))

#create folds for test sets with length varying 1,2,3,4 years but in order so 2020 then 2020-2021, then 2020-2021-2022, etc
folds <- list()
# Loop through the unique years
for (i in seq_along(years)) {
    # Create a sublist of years up to the current year
    sublist <- years[1:i]
    # Add the sublist to the folds list
    folds[[i]] <- sublist
}

# Create a array to store the results
results2 <- array(NA, dim = c( length(folds),multiStatslength,length(model_list) ), dimnames = list(as.character(folds),c("RMSE", "MAE","MAPE","R2","ME"), names(model_list)))
results2_logs <- array(NA, dim = c(length(folds), multiStatslength,length(model_list) ), dimnames = list(as.character(folds),c("RMSE", "MAE","MAPE","R2","ME"), names(model_list)))

yearchunklength=length(folds)-1
for (i in 2:yearchunklength) { #SKIP 2018 alone due to months and regions missing
  # Create the training and test sets
  train <- combined[combined$FY %in% folds[[i]], ]
  test <- combined[!combined$FY %in% folds[[i]], ]

    for (m in 1:length(model_list2)) {
    # Fit the models
    model <- lmer(model_list2[[m]], data=train)
    # Make predictions in levels not logs on test set
    pred <- exp(predict(model, newdata=test, allow.new.levels = TRUE))
    # Store the results
    results2[i,,m] <- multiStats(data.frame(pred = pred, obs = test$visits))
    results2_logs[i,,m] <- multiStats(data.frame(pred = predict(model, newdata=test, allow.new.levels = TRUE), obs = log(test$visits)))
    }
}

# show GOF results across all year chunks
overall_multi_2=data.frame(results2)
overall_multi_logs_2=data.frame(results2_logs)

# Print the summary tables. I am struggling with getting the right table format like above CV. print is working best.
print(results2)

print("inlogs")

print(results2_logs)
#kable(overall_multi_2, caption = "GOF levels... not sure R2 is interpretable this way")
#kable(overall_multi_logs_2, caption = "GOF logs") 
```