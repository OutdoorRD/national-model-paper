...
This file creates regression models for the NPS and goodness of fit routines from CV tests
...
#cut to NPS data
combined <- combined[combined$agency =="NPS",]

#create dataframe where we have visitation observations
combined=combined[complete.cases(combined$visits),]
```

## create random effects models for NPS using lme4 package (log-log) #CANT GET REGION TO WORK SINCE "GREAT PLAINS" only has one park there. Dropped regions here
...
# create random effects models for NPS using lme4 package (log-linear) needed to drop year factor for the second CV
...
#create k-fold CV test for NPS models with folds being siteids
