#########-----------------Summary Stats for NPS------------################
##This file creates scatter plots, skittles plots and scalar relationships for NPS 
...
cell <- cell[which(cell$Category == "NPS"),]
...
##load the NPS observed data
observed <- read.csv("./data/observed_data/NPS_monthly_observed.csv") %>%
  rename(year = Year,
         month = Month)
...
combined <- combined_obs_cell_sm %>%
  select("year", "month", "siteid", "Unit_Name" = "UNIT_NAME.x",
         "Airsage_ud" = "userdays", "RecreationVisits",
...
correlations_overall <- combined %>%
  ...
  filter(Parameter1 == "RecreationVisits_log",
         Parameter2 != "RecreationVisits_log") %>%
...
#the warnings in the above table are due to missing data (zeros) for some sites
#for All Trails data (and eBird for one site). Sites are WW II Memorial,
#Castle Clinton, Korean War Veterans, Lincoln, MLK Jr, SF Maritime, Vietnam 
#Veterans (all more urban sites that aren't really on All Trails)


# Create correlation matrix plot showing the correlations calculated above
pdf(file = "./figs/NPS/nps_correlation_matrix.pdf",
    width = 9.3,
    height = 7)
chart.Correlation(combined[,c("RecreationVisits_log", "Airsage_ud_log", "Flickr_ud_log",
                              "Twitter_ud_log", "AllTrails_ud_log",
                              "EBird_ud_log")])
dev.off()

#Same correlation matrix as above but showing years
pdf(file = "./figs/NPS/nps_correlation_matrix_by_year.pdf",
    width = 9.3,
    height = 7)
ggpairs(combined,
        columns = c("RecreationVisits_log", "Airsage_ud_log", "Flickr_ud_log",
                    "Twitter_ud_log", "AllTrails_ud_log",
                    "EBird_ud_log"),
...
ggsave("./figs/NPS/nps_overall_cor_plot.pdf", plot = overall_cor)
...
yearly_cor_line <- ggplot(combined_long) +
  scale_color_manual(values = c("cornflowerblue", "indianred3", "black"),
                     name = "Year",
                     breaks = c("2018", "2019", "Overall")) +
  geom_point(alpha = 0.03, aes(x = (value+1), y = (RecreationVisits),
  ...
  geom_smooth(method = "lm", se = F, linetype = "twodash",
              aes(x = (value+1), y = (RecreationVisits), color = "Overall")) +
  theme_bw(base_size = 20) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  facet_wrap(vars(data_source), scales = 'free')
...
scale_overall <- combined %>%
  summarise(scale_userdays=mean(Airsage_ud)/mean(RecreationVisits),
            scale_pud=mean(Flickr_ud)/mean(RecreationVisits),
            scale_tud=mean(Twitter_ud)/mean(RecreationVisits),
            scale_aud=mean(AllTrails_ud)/mean(RecreationVisits),
            scale_eud=mean(EBird_ud)/mean(RecreationVisits),
            nobs=n())   
...
skittles_year <- ggplot(yr) +
  ...
  xlab("Correlation with National Park Visits (log)") +
  ggtitle("Correlation by Year")


ggsave("./figs/NPS/nps_skittles_year.pdf", plot = skittles_year)
...
ggsave("./figs/NPS/nps_skittles_site.pdf", plot = skittles_site)
